...
function Watch() {
  const { watch, whishlist, setWhishlist, addcart, setAddCart, loguser} =
  ...
  const addToWhishlist = (product) => {
    if(loguser && Object.keys(loguser).length>0){
      if (whishlist.includes(product)) {
        setWhishlist(whishlist.filter((watch) => watch === product));
      } else {
 ...
 const addToCart = (product) => {
   ...
   if (loguser && Object.keys(loguser).length > 0) {
     const existingProduct = addcart.find((item) => item.Name === product.Name);
     if (existingProduct) {
       nav("/cart");
     } else {
