...
function AdminHeadphone() {
  const { headphone, setHeadphone, setProductEdit } = useContext(MyContext);
  ...
  const handleEdit = (product) => {
    ...
    nav("/edit-product-headphone", { state: { productedit: product } });
  };
  ...
  const handleDelete = (product) => {
    ...
    if (confirmed) {
      const updatedLap = headphone.filter((p) => p.Name !== product.Name);
      setHeadphone(updatedLap);
    }
  };
  const handleAddProduct = () => {
    ...
    setHeadphone((prevHeadphone) => [...prevHeadphone, newProduct]);
    setNewProduct({
      // id: Date.now(),
      Company: "",
      Name: "",
