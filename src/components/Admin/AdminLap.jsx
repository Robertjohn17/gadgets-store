...
function AdminLap() {
  const { lap, setLap, setProductEdit } = useContext(MyContext);
  ...
  const handleEdit = (product) => {
    ...
    nav("/edit-product-lap", { state: { productedit: product } });
  };
  ...
  const handleDelete = (product) => {
    ...
    if (confirmed) {
      const updatedLap = lap.filter((p) => p.Name !== product.Name);
      setLap(updatedLap);
    }
  };
  const handleAddProduct = () => {
    ...
    setLap((prevLap) => [...prevLap, newProduct]);
    setNewProduct({
      // id: Date.now(),
      Company: "",
      Name: "",
    ...
    console.log("Updated Lap:", lap);
  };
  const handleImageChange = (e) => {
    const file = e.target.files[0];
    setNewProduct({ ...newProduct, image: file });
  };
  console.log("imgages", lap);
      ...
      <div className={styles.container}>
        {lap.map((data) => (
          <div className={styles.main} key={data.Name}>
            <img
              src={data.image instanceof File ? URL.createObjectURL(data.image) : `images/${data.image}`}
              alt="img"
